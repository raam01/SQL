...
SELECT name, CONCAT(`firstname`," ",`surname`) as mem_name,starttime,
CASE WHEN Bookings.`memid`!=0 THEN `slots`*`membercost` ELSE `slots`*`guestcost` END as `cost`
FROM Bookings
INNER JOIN Facilities on Bookings.`facid`=Facilities.`facid`
INNER JOIN Members on Bookings.`memid`=Members.`memid`
WHERE Bookings.`starttime` BETWEEN '2012-09-14 00:00:00' AND '2012-09-14 23:59:59' AND (CASE WHEN Bookings.`memid`!=0 THEN `slots`*`membercost` ELSE `slots`*`guestcost` END >30)
ORDER BY `cost` DESC
